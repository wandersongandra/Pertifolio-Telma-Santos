...
function collectInlineScriptBodies(html) {
  ...
  while (cursor < html.length) {
    ...
    if (openEnd === -1) throw new Error("tag <script> sem fechamento no HTML exportado");

    const closeStart = findClosingScript(lowerHtml, openEnd + 1);
    if (closeStart === -1) throw new Error("tag <script> sem </script> correspondente no HTML exportado");

    const closeEnd = findTagEnd(html, closeStart + "</script".length);
    if (closeEnd === -1) throw new Error("tag </script> sem fechamento no HTML exportado");
...
if (!fs.existsSync(headersPath)) {
  throw new Error("out/_headers não existe. public/_headers deve ser copiado pelo build.");
}

const htmlFiles = collectHtmlFiles(outDir);
if (!htmlFiles.length) throw new Error("nenhum HTML exportado foi encontrado para geração da CSP");
...
if (!scriptHashes.size) throw new Error("nenhum script inline foi encontrado para geração da CSP");
...
if (updated === source) throw new Error("linha Content-Security-Policy não encontrada em out/_headers");

fs.writeFileSync(headersPath, updated);
console.log(
  `CSP estrita gerada com ${sortedHashes.length} hashes SHA-256 para ${htmlFiles.length} arquivos HTML.`
);
